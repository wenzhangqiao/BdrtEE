...
class TriggerRec(nn.Module):
    ...
    def forward(self, query_emb, text_emb, mask):
        ...
        :return: [b, t, 1], [], []
        '''  
        h_cln = self.ConditionIntegrator(text_emb, query_emb)#[8, 400, 768]
...
class BdrtEE(nn.Module):
    def __init__(self, config, model_weight, pos_emb_size):
        ...
        #self.loss_0 = nn.CrossEntropyLoss(reduction='none')
        #self.loss_1 = nn.CrossEntropyLoss(reduction='none')
        #self.loss_2 = nn.CrossEntropyLoss(reduction='none')
